...
/* cuda编程总结：参考https://blog.csdn.net/qq_21368481/article/details/85722590
参考https://blog.csdn.net/qq_21368481/article/details/85722590 (对该段解释)
参考https://blog.csdn.net/sunmc1204953974/article/details/51000970 (系列教程)
1. 限定词：
    __device__表示在设备上执行(GPU)，只能被设备调用
    __global__表示在设备上执行(GPU)，可由主机调用，也可由计算能力为3的设备调用
...
// cudaError_t是cuda中的一个类，用来记录cuda错误（所有cuda函数几乎都会返回一个cudaError_t）
// 这里定义一个函数用来获取返回值，从而判断是否报错，如果报错则打印出来
...
// 这里是定义threadsPerBlock作为每个block所包含的线程数目
// unsigned long long是c语言中精度最高的数据类型，64位精度，
int const threadsPerBlock =
    sizeof(unsigned long long) * 8 *
    MULTIPLIER;  // number of bits for a long long variable

// 计算iou：输入a为(m,4),b为(n,4)
__device__ inline float devIoU(float const* const a, float const* const b) {
    float left = max(a[0], b[0]), right = min(a[2], b[2]);
    float top = max(a[1], b[1]), bottom = min(a[3], b[3]); // 计算交集的left_top, right_bottom
    float width = max(right - left + 1, 0.f),
          height = max(bottom - top + 1, 0.f);   // 计算交集的w,h
    float interS = width * height;          // 计算交集的面积
    float Sa = (a[2] - a[0] + 1) * (a[3] - a[1] + 1);  //计算a的面积
    float Sb = (b[2] - b[0] + 1) * (b[3] - b[1] + 1);  //计算b的面积
    return interS / (Sa + Sb - interS);    //计算iou
}

// 函数nms_kernel，形参：
//      n_boxes: 输入预测bboxes的个数
//      nms_overlap_thrsh: iou阈值，大于该阈值则去除
//      dev_boxes: 存储bbox信息[xmin, ymin, xmax, ymax, class]
//      dev_mask: 存储iou是否超过阈值的信息，采用ULL类型表示，超过阈值设置为1，否则为0
...
void _set_device(int device_id) {
    ...
    CUDA_CHECK(cudaSetDevice(device_id));   // 执行设置cuda device的操作，并检查是否返回错误
}
...
// 这个是nms计算的核心函数：gpu_nms就是调用这个函数进行nms计算
