...
def kmeans(boxes, k, dist=np.median):
    ...
    while True:
        for row in range(rows):
            distances[row] = 1 - iou(boxes[row], clusters) # (m, k) 计算所有点跟种子点的距离，这里距离公式为d = 1 - iou，也就是覆盖度越大，距离越近，覆盖度越小，距离越远。
        ...
        for ki in range(k):
            clusters[ki] = dist(boxes[nearest_clusters == ki], axis=0)   # 更换种子点：从所有点里边获取跟该种子点已聚类的所有点，对这些点求平均，作为新的种子点，
                                                                                   # 新的种子点必然更靠近相关聚类点 
...
def avg_area_iou(boxes, clusters):
    """计算聚类的k个bbox跟整个数据集的bboxes的平均面积iou: 
    注意每个bbox跟聚类的k个bbox都可以有k个iou，其中iou最大的就是这个bbox对应的iou，因为所聚类的bbox会密布整张图，iou最大那个就会匹配到。
    args:
        bbox: (m, 2)
    ...
    return:
        avg_iou (item)
    """
    avg_iou = np.mean([np.max(iou(boxes[i], clusters)) for i in range(boxes.shape[0])])
    return avg_iou
...
if __name__ =='__main__':
    ...
    print("dataset avg ious: {:.2f}%".format(avg_iou(data, out) * 100))  # 输出数据集的平均ious
    print("kmean anchores w h:\n {}".format(out))  # 输出聚类出的k个anchor的w,h
    
    ratios = np.around(out[:, 0] / out[:, 1], decimals=2).tolist()  # 输出w/h的比例
