...
"""注意：直接采用onnx官网上的yolov3现成的onnx无法进行inference(可能跟onnx版本以及tensorRT版本相关)
采用这个变换程序从yolov3原始权重转换出来的onnx则可以进行tensorRT的inference.
这个变换程序需要做一些修改：自己下载权重和配置文件，取消onnx.check直接保存onnx文件
"""
...
#        'b969a43a848bbf26901643b833cfb96c')
    """由于下载不下来，自己找了yolov3.cfg，但这个文件无法在generate_md5_checksum()这个函数中进行md5验证，主要是py3不兼容里边hashlib.md5这个函数。
    所以我把验证也注释掉了。改用自己找到的yolov3.cfg"""
    ...
    layer_configs = parser.parse_cfg_file(cfg_file_path)  # 解析cfg文件：生成一个ordereddict, keys从000到107
    ...
    output_tensor_dims['082_convolutional'] = [255, 19, 19]  # 定义3个输出层的尺寸
...
#        'c84e5b99d0e52cd466ae710cadf6d84c')
    """这里手动把权重下载到本地"""
    weights_file_path = "/home/ubuntu/MyWeights/yolo/yolov3.weights"
