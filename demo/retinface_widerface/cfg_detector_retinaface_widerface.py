...
backbone = dict(
        type='mobilenet_v1',
        params=dict(
                pretrained= '/media/ubuntu/4430C54630C53FA2/SuLiang/MyWeights/retinaface_backbone/mobilenetV1X0.25_pretrain.tar',   # 这是caffe的模型，对应mean=[123.675, 116.28, 103.53], std=[1, 1, 1],  另外的pytorch的模型pretrained='/home/ubuntu/.torch/models/vgg16-397923af.pth', 对应mean, std需要先归一化再标准化
                out_stages=(1,2,3)))
...
transform = dict(
        aug_params=dict(
                crop_size=img_size),
        img_params=dict(
                mean=[104, 117, 123],  # 基于RGB顺序: 该数据来自retinaface的mean, std没有则为1，且明确为rgb顺序
                std=[1, 1, 1],
