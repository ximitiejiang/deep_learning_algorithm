...
class TestImg(Tester):
    ...
    def run(self, img_path):
        if isinstance(img_path, str):
            ...
            # run
            _ = self.run_single(self.model, img, data, show=True, saveto='result.jpg')
            ...
            for i, p in enumerate(img_path):
                ...
                img = cv2.imread(p)
                data = self.preprocess_data(self.cfg, img)
                result_name = p.split('/')[-1].split('.')[-2] + '_result'
                _ = self.run_single(self.model, img, data, show=True, saveto=result_name)
        else:
...
#        cfg, model = self.init_cfg_model()
        ...
        while True:
            ...
            all_results = self.run_single(self.model, img, data, show=False, saveto=None)
...
#        self.model = self.model.to(self.device)
    ...
    def preprocess_data(self, dataset_class):
        self.dataset = get_dataset(self.cfg, dataset_class)
    ...
    def run(self):
        ...
        if self.cfg.get('cudnn_benchmark', False):
            ...
            outputs = self.run_single(self.dataloader, show=False)  # (4870,)=n_imgs (20,)=n_classes (n,5)=n_bboxes
...
if __name__ == "__main__":
    ...
    test_dataset = TestDataset(config_file=config_path, 
